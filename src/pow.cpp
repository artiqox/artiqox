...
bool AllowMinDifficultyForBlock(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Artiqox: Magic number at which reset protocol switches
    // check if we allow minimum difficulty at this block-height
    if (pindexLast->nHeight < 60)
        return false;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Artiqox: Special rules for minimum difficulty blocks with Digishield
    ...
    return CalculateArtiqoxNextWorkRequired(pindexLast, pindexFirst->GetBlockTime(), params);
}
