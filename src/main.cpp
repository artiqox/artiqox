...
CFeeRate minRelayTxFee = CFeeRate(COIN / 2);
...
{
    return nTime + Params().NetBlockTimeout() * (4 + nValidatedQueuedBefore);
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Artiqox: BIP16 has been active since inception
    ...
    CAmount blockReward = nFees + GetArtiqoxBlockSubsidy(pindex->nHeight, consensus, hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Artiqox: Version 2 enforcement was never used, reject from v3 softfork
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Artiqox: Block v2 was never enforced, so we trigger this against v3
