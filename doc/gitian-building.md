...
In [doc/release-process.md](release-process.md) in the artiqox repository under 'Fetch and build inputs'.
...
To build Artiqox (for Linux and/or Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/langerhans/artiqox.git
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/artiqox-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/artiqox-build.assert
```
...
[artiqox/gitian.sigs](https://github.com/artiqox/gitian.sigs/) repository, or
