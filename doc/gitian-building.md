...
*Setup instructions for a gitian build of Artiqox using a Debian VM or physical system.*
...
- [Building Artiqox](#building-artiqox)
...
Clone the git repositories for artiqox and gitian.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the artiqox repository
...
To build Artiqox (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the artiqox repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/artiqox/.git/
...
For example:
```bash
URL=https://github.com/laanwj/artiqox.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/artiqox-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/artiqox-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/artiqox-osx-build.assert
```
...
[artiqox/gitian.sigs](https://github.com/artiqox/gitian.sigs/) repository, or if that's not possible create a pull
request.
