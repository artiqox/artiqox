...
class GetAuxBlockTest (BitcoinTestFramework):
  ...
  def run_test (self):
    ...
    assert_equal (auxblock['chainid'], 42)
    ...
    apow = auxpow.computeAuxpowWithChainId (auxblock['hash'], target, "42", False)
    res = self.nodes[0].getauxblock (auxblock['hash'], apow)
    assert not res

    # Compute and submit valid auxpow.
    apow = auxpow.computeAuxpowWithChainId (auxblock['hash'], target, "42", True)
    res = self.nodes[0].getauxblock (auxblock['hash'], apow)
    assert res
    ...
    assert_equal ("01%02x01" % auxblock['height'], coinbase[0 : 6]) # AIQ: We mine less blocks in these tests
